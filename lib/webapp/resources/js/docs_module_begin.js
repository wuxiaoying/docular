...
docsApp.serviceFactory.contentLoading = function () {
...
docsApp.controller.DocsController = function($scope, $location, $window, $cookies, sections, contentLoading) {
    ...
    var MODULE_FILTER = /^(.+)\.filter:([^\.]+)$/;
    //var MODULE_SERVICE = /^((?:(?!^angular\.)[^\.])+)\.([^\.]+?)(Provider)?$/;
    var MODULE_SERVICE = /^(.+)\.([^\.]+?)(Provider)?$/;
    ...
    $scope.$watch(function docsPathWatch() {return $location.path(); }, function docsPathWatchAction(path) {
        ...
        if (DOCS_PATH.test(path)) {
            ...
            if(isGroupIndexPage){
                ...
                var pageAlreadyLoaded = $scope.currentPage && page && $scope.currentPage.partialUrl ? (page.partialUrl == $scope.currentPage.partialUrl ? true : false) : false;
            ...
            if (!module) {
                ...
                module = cache[name] = {
                    ...
                    section: function(page) {
                        if(page.moduleSection == "booyah"){
                            console.log("BOOYAH:", page);
                        }
