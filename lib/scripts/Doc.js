...
Doc.prototype = {
    ...
    */
    ...
    parse: function() {
        ...
        var allLines;
        try {
            allLines = self.text.split(NEW_LINE);
        } catch (e) {
            console.log("ERROR: ".red + " Parsing document text | file: " + self.file + " | group: " + self.group + " | section: " + self.section);
        }
        ...
        allLines.forEach(function(line){
            ...
            if ((match = line.match(/^\s*@((\w+)(-(\w+))*)(\s+(.*))?/))) {
                ...
                if (match[match.length - 1]) parseValue.push(match[match.length - 1]);
    ...
    html: function() {
        ...
        dom.h(self.doc_api_extensions.heading.call(self, dom), function() {
            ...
            for(var i=0; i < renderList.length; i++) {
                ...
                } catch (e) { console.log("WARNING: ".yellow + " Error rendering expanded render method '" + renderList[i].id + "' ", e); }
            }
